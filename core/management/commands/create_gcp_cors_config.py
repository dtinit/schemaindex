...
class Command(BaseCommand):
    help = (
        "Create gcp-cors-config.json file for Google Cloud Storage CORS configuration"
    )
    ...
    def handle(self, *args, **options):
        ...
        if not settings.GS_BUCKET_NAME:
            self.stdout.write(self.style.ERROR("GS_BUCKET_NAME is not defined"))
            return
        ...
        self.stdout.write(self.style.SUCCESS("gcp-cors-config.json created"))
