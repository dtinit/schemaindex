    ...
    def clean_url(self):
        ...
        if not matched_language:
            raise ValidationError(
                "The provided URL does not have a supported file extension"
            )
        ...
        for schema_ref in schema_refs:
            if schema_ref.url_provider_info.is_same_resource(url):
                raise ValidationError(
                    "The provided URL is already in use by another Schema"
                )
        ...
        for schema_ref in schema_refs:
            if schema_ref.id_value == id_value:
                raise ValidationError(
                    "A JSON schema with this resource's $id is already in use by another Schema"
                )
...
class DocumentationItemForm(ReferenceItemForm):
    ...
    role = forms.ChoiceField(
        ...
        + [
            role
            for role in DocumentationItem.DocumentationItemRole.choices
            if role[0]
            not in (
    ...
    format = forms.ChoiceField(
        choices=[("", "Other")]
        + list(DocumentationItem.DocumentationItemFormat.choices),
        required=False,
        label="Format",
        initial="",
    )
    ...
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.fields["format"].widget.attrs["data-url-format-selector-for"] = self[
            "url"
        ].id_for_label
...
class ImplementationForm(ReferenceItemForm):
    is_open_source = forms.BooleanField(
        label="This implementation is open source", required=False
    )
...
class SchemaForm(forms.Form):
    ...
    name = forms.CharField(
        label="Name", max_length=200, help_text="A descriptive name for your schema"
    )
    ...
    def __init__(self, *args, schema=None, **kwargs):
        ...
        other_documentation_items = schema.documentationitem_set.exclude(
            id__in=[
                ref.id for ref in (latest_readme, latest_license) if ref is not None
            ]
        )
        ...
        SchemaRefFormsetFactory = forms.formset_factory(
            SchemaRefForm, extra=extra_formset_item_count
        )
        self.schema_refs_formset = SchemaRefFormsetFactory(
            prefix="schema_refs",
    ...
    def is_valid(self):
        is_documentation_items_formset_valid = (
            self.additional_documentation_items_formset.is_valid()
        )
        ...
        return (
            is_form_valid
            and is_documentation_items_formset_valid
            and is_schema_refs_formset_valid
            and is_implementation_formset_valid
        )
...
class PermanentURLForm(forms.Form):
    ...
    link_type = forms.ChoiceField(
        label="URL",
        widget=forms.Select(
            attrs={"class": "js-autorefresh-with-value input-group__prefix"}
        ),
    )
    ...
    def __init__(self, *args, schema, **kwargs):
        ...
        for schema_ref in schema.schemaref_set.all():
            target_choices.append((
                f"schemaref:{schema_ref.id}",
                f"{schema_ref.name or schema_ref.url} (Definition)",
            ))
    ...
    def clean_target(self):
        ...
        if (
            (target_type != "schema" and target_type != "schemaref")
            or (
                target_type == "schema"
                and not Schema.objects.filter(id=target_id).exists()
            )
            or (
                target_type == "schemaref"
                and not SchemaRef.objects.filter(id=target_id).exists()
            )
        ):
    ...
    def clean(self):
        ...
        if (
            PermanentURL.objects.filter(created_by=self.schema.created_by).count()
            >= MAX_PERMANENT_URL_COUNT_PER_USER
        ):
            raise ValidationError(
                f"You have reached the limit of {MAX_PERMANENT_URL_COUNT_PER_USER} permanent URLs for your account."
            )
        ...
        if link_type == self.LinkType.EMAIL:
            ...
            self.schema.created_by.profile.organization
        ):  # link_type == self.LinkType.ORGANIZATION
