...
def test_schema_management_form_prevents_duplicate_published_id_values():
    ...
    with requests_mock.Mocker() as m:
        ...
        SchemaRefFactory(url=existing_schema_ref_url, id_value=mock_id_value)
...
def test_permanent_url_form_prevents_duplicate_urls(link_type):
    ...
    PermanentURLFactory(
        content_object=other_schema, link_type=link_type, suffix=duplicate_suffix_value
    )
    form_data = {
