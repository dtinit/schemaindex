...
import json
from factories import ProfileFactory, SchemaRefFactory, SchemaFactory, UserFactory
...
def test_api_requires_valid_api_key():
    client = Client()
    response = client.get("/api/find", headers={"X-API-Key": "invalid key"})
    assert response.status_code == 401
...
def test_api_key_enforces_rate_limit_on_profile():
    ...
    with requests_mock.Mocker() as m:
        ...
        for _ in range(2):
            raw_api_key = profile.set_new_api_key()
            response = client.get(
                f"/api/find?id={id_value}", headers={"X-API-Key": raw_api_key}
            )
            assert response.status_code == 200
        raw_api_key = profile.set_new_api_key()
...
def test_create_schema_with_reference_items(api_client):
    ...
    manifest = {
        ...
        "documents": {
            definition_url: {"type": "definition", "name": "Mock definition"},
            readme_url: {
                "type": "documentation",
                "name": "README.md",
                "role": "readme",
            ...
            implementation_url: {"type": "implementation", "isOpenSource": True},
        },
    }
    response = api_client.post(
        "/api/schemas", data=json.dumps(manifest), content_type="application/json"
    )
...
def test_create_rejects_non_json_payloads(api_client):
    response = api_client.post(
        "/api/schemas", data="not json", content_type="application/json"
    )
...
def test_create_rejects_non_manifest_payloads(api_client):
    response = api_client.post(
        "/api/schemas", data='{"not_a_manifest": true}', content_type="application/json"
    )
    assert response.status_code == 400
...
def test_create_prevents_publishing_schemas_with_existing_definition_urls(api_client):
    ...
    assert (
        "is already using one of the URL values used in this schema"
...
def test_update_rejects_non_json_payloads(api_client):
    ...
    response = api_client.put(
        f"/api/schemas/{schema.id}", data="not json", content_type="application/json"
    )
    assert response.status_code == 400
...
def test_update_prevents_unpublishing_schemas(api_client):
    ...
    assert response.json()["error"]["message"] == "Validation Error"
    assert (
        "Public schemas cannot be made private" in response.json()["error"]["details"]
    )
